...
def shell():
    ...
    printer('Testando do provendor da %(isp)s (%(ip)s)...' % speedtest.config['client'],
            quiet)

    if not args.mini:
        printer('Recuperando o speedtest.net carregando Lista de servidores...', quiet)
        try:
            ...
            printer('Não é possível recuperar a lista de servidores de speedtest', error=True)
            raise SpeedtestCLIError(get_exception())
        except InvalidServerIDType:
            raise SpeedtestCLIError(
                '%s é um tipo de servidor inválido, devo '
                'seja um int' % ', '.join('%s' % s for s in args.server)
            )
    ...
    if args.download:
        printer('Testando a velocidade de download\n', quiet,
                end=('', '\n')[bool(debug)])
        speedtest.download(
            callback=callback,
    ...
    if args.upload:
        printer('Testando a velocidade de upload\n', quiet,
                end=('', '\n')[bool(debug)])
        speedtest.upload(
            callback=callback,
...
def main():
    try:
        ...
        printer('\nCancelamento...', error=True)
    except (SpeedtestException, SystemExit):
        e = get_exception()
        # Ignorar uma saída bem-sucedida ou saída argparse
